...
from utils.epsilon_greedy import epsilon_greedy, EpsilonManager, boltzmann_policy, get_action_from_q, centralized_epsilon_greedy
...
def save_model_parameters_centralized(agent_net, directory):
    os.makedirs(directory, exist_ok=True)  # Ensure the directory exists
    torch.save(agent_net.state_dict(), os.path.join(directory, f'centralized_agent_params.pt'))

def load_model_parameters_centralized(agent_net, directory):
    # Load mixer parameters
    agent_state_dict = torch.load(os.path.join(directory, 'centralized_agent_params.pt'))
...
def main(config_file=None, additional_description=None):
    ...
    # buffer = ReplayBuffer(agents=agents)
    ...
    n_actions_list = [5,5,5]
    agent_net = CentralizedRNNAgent(input_shape=15, n_hidden_dim=64, n_actions_list=n_actions_list).to(device)

    if pretrained_parameters is not None:
        load_model_parameters_centralized(agent_net, directory=pretrained_parameters)
        print("Pretrained parameters successfully loaded.")

    print("Check model devices ...")
    for param in agent_net.parameters():
        print(f"Central Agent | {param.device = }")

    # Combine parameters from all relevant parts of the model
    all_params = list(agent_net.parameters())
    ...
    # Copy the agent network to use as target networks.
    target_agent_net = deepcopy(agent_net).to(device)
    ...
    for episode in range(max_episodes):
        ...
        for step in range(env.max_control_timestep):
            ...
            if step == 0:
                ...
                agent_hidden = agent_net.init_hidden()
            else:
                if not render_mode == 'silent':
                    print(f"epsilon = {epsilon:.2f}", end=' | ')

                # 2.1. Using the observation from the last timestep, decide which action to take.
                # 2.1.1. Get Q-values from the agent network.
                q, h = agent_net(torch.from_numpy(previous_state).to(device), agent_hidden)
                q = centralized_mask_and_nothing(previous_observations, q)
                agent_hidden = h
                    
                # 2.1.2. Decide actions to take with action policy (epsilon-greedy, Boltzmann, pure greedy).
                actions_list = centralized_epsilon_greedy(q, previous_observations, epsilon, n_actions_list)
                actions = {a: actions_list[j] for j, a in enumerate(agents)}
            ...
            if update_hard:
                if episodic:
                    if (episode % hard_update_frequency == 0) and step == 0:
                        ...
                        buffer.prioritize(None, None, agent_net, target_agent_net, device, env, gamma=gamma, mode=PER_mode, calculate_for_all=True)
                else:
                    assert "Use episodic."
            # Update target networks (soft update). Only hard update was used in the paper.
            else:
                assert "Use hard update."
            ...
            if train_condition:
                ...
                if episodic:
                    ...
                    buffer.prioritize(None, None, agent_net, target_agent_net, device, env, gamma=gamma, mode=PER_mode, calculate_for_all=calculate_for_all)
                ...
                for i in range(n_epoch):
                    ...
                    if episodic:
                        ...
                        for j, train_episode_id in enumerate(episodes_to_train):
                            buffer.calculate_loss(mixer=None, target_mixer=None, agent_nets=agent_net, target_agent_nets=target_agent_net,
                ...
                if episodic:
                    save_model_parameters_centralized(agent_net, directory=param_dir)
